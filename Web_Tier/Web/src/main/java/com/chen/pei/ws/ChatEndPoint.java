...
import com.chen.pei.interceptor.UserInterceptor;
import com.chen.pei.pojo.Message;
import com.chen.pei.utils.MessageUtils;
...
public class ChatEndPoint {
    ...
    //建立连接时发送系统广播
    //（因为ServerEndpointConfig 继承自 EndpointConfig 可以向该对象中 put 进入 httpsession）
    // 然后在 httpsession 中获取当前登录用户
    @OnOpen
    ...
    private void broadcastAllUsers(String message){
        try{
            ...
            for(String name : names){
                ...
                chatEndPoint.session.getBasicRemote().sendText(message); //用于传送信息
            }
        }catch (Exception e){
            e.printStackTrace();
        }
